...
File Information: This file includes plotting analysis.
    ...
    if not ims:
        ...
        if n_subplots == 1:  # Plotting only one image
            for v in voltages.items():
                ...
                plt.title("%s voltages differences for neurons (%d - %d) from t = %d to %d " % args)
                plt.xlabel("Time (ms)")
            ...
            for i, v in enumerate(voltages.items()):
                ...
                args = (v[0], n_neurons[v[0]][0], n_neurons[v[0]][1], time[0], time[1])
                axes[i].set_title(
                    "%s voltages differences for neurons (%d - %d) from t = %d to %d " % args
                )
