...
File Information: This file includes abstract Learning rules class and some implemented rules.
...
from ULIIC.network.neurons import SRM0Neurons
from ULIIC.network.synapese import (
    AbstractConnection,
    Connection,
    Conv2dConnection,
    LocalConnection,
)
from ULIIC.auxiliary.snn_utils import im2col_indices
from ULIIC.computing.cordic_exp import error_exp, conventional_cordic_exp, pipeline_cordic_exp
...
class PostPre(LearningRule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        ), "Both pre- and post-synaptic Neurons must record spike traces."
...
class WeightDependentPostPre(LearningRule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert self.source.traces, "Pre-synaptic Neurons must record spike traces."
        assert (
...
class Rmax(LearningRule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        ), "Pre-synaptic Neurons should keep track of their firing trace in an additive way."

        # Derivation of R-max depends on stochastic SRM neurons!
        assert isinstance(
            self.target, SRM0Neurons
        ), "R-max needs stochastically firing neurons, use SRM0Neurons."
...
class ExpWeightSTDP(LearningRule):
    # language=rst
    """
    exp weights dependent STDP rule involving both pre- and post-synaptic spiking activity.
    The pre-synaptic update is negative, while the post-synpatic update is positive.
    """
    ...
    def __init__(
        ...
        reduction: Optional[callable] = None,
        weight_decay: float = 0.0,
        beta: float = 0.2,  # 0.2, 1.0, 0.5...
        **kwargs
    ) -> None:
        """
        Constructor for ``ExpWeightSTDP`` learning rule.

        :param connection: An ``AbstractConnection`` object whose weights the ``PostPre`` learning rule will modify.
        :param nu: Single or pair of learning rates for pre- and post-synaptic events, respectively.
        :param reduction: Method for reducing parameter updates along the minibatch dimension.
        :param weight_decay: Constant multiple to decay weights by on each iteration.
        :param beta: Determines the strength of the weight dependence.
        """
        super().__init__(
            connection=connection,
            nu=nu,
        ...
        assert (
            self.source.traces and self.target.traces
        ), "Both pre- and post-synaptic nodes must record spike traces in this learning rule."
    ...
    def _connection_update(self, **kwargs) -> None:
        # language=rst
        """
        ExpWeight learning rule for ``Connection`` subclass of ``AbstractConnection`` class.
        """
        batch_size = self.source.batch_size
        ...
        #     update = self.reduction(torch.bmm(source_s, target_x), dim=0)
        #     self.connection.w -= self.nu[0] * update
        # update = self.reduction(source_s, dim=0) * \
        #          self.reduction(source_x, dim=0) * error_exp()()()(-self.beta * self.connection.w) - \
        #          self.reduction(torch.bmm(source_x, target_s), dim=0) * \
        #          error_exp()()()(-self.beta * (self.wmax - self.connection.w))
        # self.connection.w += self.nu[0] * update

        update = self.reduction(torch.bmm(source_x, target_s), dim=0) * \
                 error_exp(-self.beta * self.connection.w, error=self.neederror) - \
                 self.reduction(torch.bmm(source_x, target_s), dim=0) * \
                 error_exp(-self.beta * (self.wmax - self.connection.w), error=self.neederror)
        self.connection.w += self.nu[0] * update
